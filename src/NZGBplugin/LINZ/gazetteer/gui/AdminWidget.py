...
class AdminWidget(QWidget, UI_ADMIN_WIDGET):
    ...
    def downloadCsvFiles(self):
        ...
        dir = QFileDialog.getExistingDirectory(
            self, "Select download folder", downloaddir
        )
        if not dir:
            return
        dir = str(dir)
        ...
        if not exports:
            QMessageBox.information(
                self, "No files", "Currently no CSV download files are defined"
            )
            return
        ...
        if overwrite:
            ...
            result = QMessageBox.question(
                self, "Overwrite files?", msg, QMessageBox.Yes | QMessageBox.No
            )
            if result != QMessageBox.Yes:
                return
